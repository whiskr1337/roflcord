...
import { Settings } from "../src/Roflcord";
...
const setCssDebounced = debounce((css: string) => RoflcordNative.quickCss.set(css));

const themeStore = DataStore.createStore("RoflcordThemes", "RoflcordThemeData");

// probably should make this less cursed at some point
window.RoflcordNative = {
    themes: {
    ...
    updater: {
        getRepo: async () => ({ ok: true, value: "https://github.com/Vendicated/Roflcord" }),
    ...
    quickCss: {
        get: () => DataStore.get("RoflcordQuickCss").then(s => s ?? ""),
        set: async (css: string) => {
            await DataStore.set("RoflcordQuickCss", css);
        ...
        async openEditor() {
            ...
            const win = open("about:blank", "RoflcordQuickCss", features);
            if (!win) {
            ...
            win.getCurrentCss = () => RoflcordNative.quickCss.get();
            win.getTheme = () =>
    ...
    settings: {
        get: () => {
            try {
                return JSON.parse(localStorage.getItem("RoflcordSettings") || "{}");
            } catch (e) {
                console.error("Failed to parse settings from localStorage: ", e);
                return {};
            }
        },
        set: async (s: Settings) => localStorage.setItem("RoflcordSettings", JSON.stringify(s)),
