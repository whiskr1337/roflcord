...
const BASE_URL = "https://github.com/Roflcord/Installer/releases/latest/download/";
const INSTALLER_PATH_DARWIN = "RoflcordInstaller.app/Contents/MacOS/RoflcordInstaller";
...
function getFilename() {
    switch (process.platform) {
        case "win32":
            return "RoflcordInstallerCli.exe";
        case "darwin":
            return "RoflcordInstaller.MacOS.zip";
        case "linux":
            return "RoflcordInstallerCli-linux";
        default:
...
async function ensureBinary() {
    ...
    const outputFile = process.platform === "darwin"
        ? join(FILE_DIR, "RoflcordInstaller")
        : downloadName;
    ...
    const res = await fetch(BASE_URL + filename, {
        headers: {
            "User-Agent": "Roflcord (https://github.com/Vendicated/Roflcord)",
            "If-None-Match": etag
        }
    });
    ...
    if (process.platform === "darwin") {
        ...
        logAndRun(`sudo spctl --add '${outputFile}' --label "Roflcord Installer"`);
...
try {
    execFileSync(installerBin, args, {
        ...
        env: {
            ...process.env,
            ROFLCORD_USER_DATA_DIR: BASE_DIR,
            ROFLCORD_DEV_INSTALL: "1"
        }
    });
} catch {
