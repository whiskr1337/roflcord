...
const commonOptions = {
    ...commonOpts,
    entryPoints: ["browser/Roflcord.ts"],
    format: "iife",
    globalName: "Roflcord",
...
const buildConfigs = [
    ...
    },
    {
        ...
        footer: { js: "//# sourceURL=file:///RoflcordWeb" }
    },
    {
        ...commonOptions,
    ...
    },
    {
        ...
        outfile: "dist/Roflcord.user.js",
        banner: {
            js: readFileSync("browser/userscript.meta.js", "utf-8").replace("%version%", `${VERSION}.${new Date().getTime()}`)
        },
        footer: {
            // UserScripts get wrapped in an iife, so define Roflcord prop on window that returns our local
            js: "Object.defineProperty(unsafeWindow,'Roflcord',{get:()=>Roflcord});"
        }
    }
];
...
async function buildExtension(target, files) {
    const entries = {
        "dist/Roflcord.js": await readFile("dist/extension.js"),
        "dist/Roflcord.css": await readFile("dist/extension.css"),
...
const appendCssRuntime = readFile("dist/Roflcord.user.css", "utf-8").then(content => {
    const cssRuntime = `
;document.addEventListener("DOMContentLoaded", () => document.documentElement.appendChild(
    Object.assign(document.createElement("style"), {
        textContent: \`${content.replaceAll("`", "\\`")}\`,
        id: "roflcord-css-core"
    })
), { once: true });
`;

    return appendFile("dist/Roflcord.user.js", cssRuntime);
});
