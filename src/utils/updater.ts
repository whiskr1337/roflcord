...
export async function checkForUpdates() {
    changes = await Unwrap(RoflcordNative.updater.getUpdates());
...
export async function update() {
    ...
    const res = await Unwrap(RoflcordNative.updater.update());

    if (res) {
        isOutdated = false;
        if (!await Unwrap(RoflcordNative.updater.rebuild()))
...
export const getRepo = () => Unwrap(RoflcordNative.updater.getRepo());
