...
export const SpotifyStore = proxyLazyWebpack(() => {
    ...
    class SpotifyStore extends Store {
        ...
        public openExternal(path: string) {
            const url = Settings.plugins.SpotifyControls.useSpotifyUris || Roflcord.Plugins.isPluginEnabled("OpenInApp")
                ? "spotify:" + path.replaceAll("/", (_, idx) => idx === 0 ? "" : ":")
                : "https://open.spotify.com" + path;

            RoflcordNative.native.openExternal(url);
        }
        ...
        seek(ms: number) {
            ...
            return this._req("put", "/seek", {
                ...
                console.error("[RoflcordSpotifyControls] Failed to seek", e);
