...
function makeShortcuts() {
    ...
    return {
        ...
        wpPatcher: { getter: () => Roflcord.WebpackPatcher },
        wpInstances: { getter: () => Roflcord.WebpackPatcher.allWebpackInstances },
        wpsearch: search,
        wpex: extract,
        ...
        PluginsApi: { getter: () => Roflcord.Plugins },
        plugins: { getter: () => Roflcord.Plugins.plugins },
        Settings: { getter: () => Roflcord.Settings },
        Api: { getter: () => Roflcord.Api },
        Util: { getter: () => Roflcord.Util },
            ...
            if (!win.prepared) {
                ...
                [...document.querySelectorAll("style"), ...document.querySelectorAll("link[rel=stylesheet]")].forEach(s => {
                    ...
                    if (s.parentElement?.tagName === "HEAD")
                        doc.head.append(n);
                    else if (n.id?.startsWith("roflcord-") || n.id?.startsWith("vcd-"))
        ...
        preEnable: (plugin: string) => (Roflcord.Settings.plugins[plugin] ??= { enabled: true }).enabled = true,
...
function loadAndCacheShortcut(key: string, val: any, forceLoad: boolean) {
    ...
    function unwrapProxy(value: any) {
        ...
        } else if (value.$$roflcordGetWrappedComponent) {
            return forceLoad ? value.$$roflcordGetWrappedComponent() : value;
        }
...
export default definePlugin({
    ...
    start() {
        ...
        if (!IS_WEB) {
            const Native = RoflcordNative.pluginHelpers.ConsoleShortcuts as PluginNative<typeof import("./native")>;
