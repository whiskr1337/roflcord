...
function RoflcordPopout(onClose: () => void) {
    const { useQuickCss } = useSettings(["useQuickCss"]);

    const pluginEntries = [] as ReactNode[];

    for (const plugin of Object.values(Roflcord.Plugins.plugins)) {
        if (plugin.toolboxActions && Roflcord.Plugins.isPluginEnabled(plugin.name)) {
            pluginEntries.push(
                <Menu.MenuGroup
                    label={plugin.name}
    ...
    return (
        ...
        >
            ...
            <Menu.MenuItem
                ...
                label="Open QuickCSS"
                action={() => RoflcordNative.quickCss.openEditor()}
            />
            {...pluginEntries}
        </Menu.Menu>
    );
...
function RoflcordPopoutIcon(isShown: boolean) {
    return (
...
function RoflcordPopoutButton({ buttonClass }: { buttonClass: string; }) {
    ...
    return (
        <Popout
            ...
            renderPopout={() => RoflcordPopout(() => setShow(false))}
        >
            {(_, { isShown }) => (
                <HeaderBarIcon
                    ref={buttonRef}
                    ...
                    onClick={() => setShow(v => !v)}
                    tooltip={isShown ? null : "Roflcord Toolbox"}
                    icon={() => RoflcordPopoutIcon(isShown)}
                    selected={isShown}
                />
            )}
        </Popout>
...
export default definePlugin({
    name: "RoflcordToolbox",
    description: "Adds a button to the titlebar that houses Roflcord quick actions",
    ...
    patches: [
        {
            ...
            replacement: {
                ...
                replace: "$& $self.renderRoflcordPopoutButton($1),"
            }
        }
    ],

    renderRoflcordPopoutButton: (buttonClass: string) => (
        <ErrorBoundary noop>
            <RoflcordPopoutButton buttonClass={buttonClass} />
        </ErrorBoundary>
    )
});
