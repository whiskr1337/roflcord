            ...
            replacement: [
                {
                    ...
                    replace: "if(Roflcord.Api.Notices.currentNotice)return false;$&"
                },

                // FIXME(Bundler minifier change related): Remove the non used compability once enough time has passed
                {
                    match: /(?<=,NOTICE_DISMISS:function\(\i\){)return null!=(\i)/,
                    replace: (m, notice) => `if(${notice}?.id=="RoflcordNotice")return(${notice}=null,Roflcord.Api.Notices.nextNotice(),true);${m}`,
                    noWarn: true,
                },
                {
                    match: /(?<=function (\i)\(\i\){)return null!=(\i)(?=.+?NOTICE_DISMISS:\1)/,
                    replace: (m, _, notice) => `if(${notice}?.id=="RoflcordNotice")return(${notice}=null,Roflcord.Api.Notices.nextNotice(),true);${m}`
                }
            ]
        }
    ],
