...
async function runReporter() {
    ...
    try {
        ...
        addPatch({
            ...
            replacement: {
                match: /(?<="use strict";)/,
                replace: "Roflcord.Webpack._initReporter();"
            }
        }, "Roflcord Reporter");

        // @ts-expect-error
        Roflcord.Webpack._initReporter = function () {
        ...
        for (const [searchType, args] of Webpack.lazyWebpackSearchHistory) {
            ...
            try {
                ...
                if (result == null || (result.$$roflcordGetWrappedComponent != null && result.$$roflcordGetWrappedComponent() == null)) throw new Error("Webpack Find Fail");
            } catch (e) {
                let logMessage = searchType;
                if (method === "find" || method === "proxyLazyWebpack" || method === "LazyComponentWebpack") {
                    if (args[0].$$roflcordProps != null) {
                        logMessage += `(${args[0].$$roflcordProps.map(arg => `"${arg}"`).join(", ")})`;
                    } else {
...
// Run after the Roflcord object has been created.
// We need to add extra properties to it, and it is only created after all of Roflcord code has ran
