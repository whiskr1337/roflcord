...
import { ApplicationCommandInputType, ApplicationCommandOptionType, ApplicationCommandType, RoflcordCommand } from "./types";

export * from "./commandHelpers";
export * from "./types";

export let BUILT_IN: RoflcordCommand[];
export const commands = {} as Record<string, RoflcordCommand>;
...
export const _init = function (cmds: RoflcordCommand[]) {
    try {
        BUILT_IN = cmds;
...
export const _handleCommand = function (cmd: RoflcordCommand, args: CommandArgument[], ctx: CommandContext) {
    if (!cmd.isRoflcordCommand)
 ...
 * @param opt
 */
export function prepareOption<O extends CommandOption | RoflcordCommand>(opt: O): O {
...
// investigate
function registerSubCommands(cmd: RoflcordCommand, plugin: string) {
...
export function registerCommand<C extends RoflcordCommand>(command: C, plugin: string) {
    if (!BUILT_IN) {
        console.warn(
            "[CommandsAPI]",
    ...
    command.isRoflcordCommand = true;
