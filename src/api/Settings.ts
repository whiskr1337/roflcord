...
const DefaultSettings: Settings = {
    ...
    cloud: {
        authenticated: false,
        url: "https://api.roflcord.dev/",
        settingsSync: false,
        settingsSyncVersion: 0
    }
};

const settings = !IS_REPORTER ? RoflcordNative.settings.get() : {} as Settings;
...
if (!IS_REPORTER) {
    SettingsStore.addGlobalChangeListener((_, path) => {
        ...
        localStorage.Roflcord_settingsDirty = true;
        saveSettingsOnFrequentAction();
        RoflcordNative.settings.set(SettingsStore.plain, path);
    });
}
